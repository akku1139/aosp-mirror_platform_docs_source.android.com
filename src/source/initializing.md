...
version of the source code you plan to compile. See [Build Numbers](build-
numbers.html) for a full listing of branches you may choose from. You may also
choose to download and build the latest source code (called "master"), in which
case you will simply omit the branch specification when you initialize the
repository.
...
Xcode is available from [the Apple developer site](http://developer.apple.com/).
If you are not already registered as an Apple developer, you will have to create
an Apple ID to download Xcode.

The remaining sections for Mac OS X only apply to those who wish to build earlier branches.

## Branch 4.0.3x and all earlier branches ##

To build android-4.0.3x and earlier branches in a Mac OS environment, you need
an Intel/x86 machine running MacOS 10.5 (Leopard) or MacOS 10.6 (Snow Leopard).
You will need the MacOS 10.5 SDK.
