...
Android Pony EXpress (APEX) is a container format introduced in Android
...
([HALs](/devices/architecture/hal-types)), runtime
([ART](/devices/tech/dalvik)), and class libraries.
...
For more information on why this design for APEX was selected, see
[Alternatives considered when developing APEX](#alternatives_considered_when_developing_apex).

### APEX format

This is the format of an APEX file.

![APEX file format](/devices/tech/ota/images/apex-format.png)
...
1.  At reboot, the APEX manager starts, reads the internal database, and does
    ...
    1.  Creates a loopback device from the APEX file.
    1.  Creates a device mapper block device on top of the loopback device.
...
This does not mean that APEX can update all of these file types. Whether a file
...
[APK signature scheme v3](/security/apksigning/v3). Two different keys are used
in this process.
...
The performance of the loopback driver and dm-verity is important in achieving
...
higher. New devices launching with Android {{ androidQVersionNumber }} or higher
...
Android {{ androidQVersionNumber }} and want to support APEX modules. To get the
...
APEX modules that were introduced in Android {{ androidQVersionNumber }}. The
...
<td><code>/lib</code> and <code>/lib64</code> (<code>/lib/arm</code> for
...
<code class="devsite-terminal">openssl genrsa -out foo.pem 4096</code>

# extract the public key from the key pair
<code class="devsite-terminal">avbtool extract_public_key --key foo.pem --output foo.avbpubkey</code>
...
<pre class="prettyprint">
<code class="devsite-terminal">adb install <var>apex_file_name</var></code>
...
Flattened APEX is the default configuration. This means that all
APEXs are by default flattened unless you explicitly configure your device
to build non-flattened APEXs to support APEX updates (as explained above).

Mixing flattened and non-flattened APEXs in a device is NOT
...
device should inherit from `updatable_apex.mk` as explained in
[Updating a service with an APEX](#updating_a_service_with_an_apex).
